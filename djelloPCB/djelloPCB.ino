...
void setup(void) {
  ...
  pinMode(btnVolUpPin, INPUT);
  pinMode(btnVolDownPin, INPUT);
...
// T1, T2 not possible
int touchPins[] = {T0, T1, T2, T3, T4, T5, T6, T7, T8, T9};
int touchThreshold[] = {60, 0, 0, 60, 60, 60, 60, 40, 40, 20};
int prevTouch[] = {false, false, false, false, false, false, false, false, false, false};
int currTouch[10];
...
void loop() {
  for(int i = 0; i < 10; i++) {
    ...
    currTouch[i] = touch;
    Serial.print("T" + String(i) + "=" + String(touch) + ", ");
  }
  Serial.println();

  for(int i = 0; i < 10; i++) {
    if (currTouch[i] < touchThreshold[i] && !prevTouch[i]) {
      Serial.println("TOUCHED " + String(i));
      // DF1201S.playFileNum(i+1);
      DF1201S.playSpecFile("/sound" + String(i+1) + ".mp3");
      DF1201S.start();
      prevTouch[i] = true;
      delay(1000);
    } 

    prevTouch[i] = currTouch[i] < touchThreshold[i];
  }

  /* VOLUME BUTTONS */
  int btnVolUp = digitalRead(btnVolUpPin);
  int btnVolDown = digitalRead(btnVolDownPin);

  if (btnVolUp == HIGH) {
    DF1201S.setVol(min(DF1201S.getVol() + 1, 30));
    Serial.println("VOL UP: ");
    Serial.println(DF1201S.getVol());
    delay(250);
  }
  if (btnVolDown == HIGH) {
    DF1201S.setVol(max(DF1201S.getVol() - 1, 0));
