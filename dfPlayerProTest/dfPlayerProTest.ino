...
void setup(void)
{
  Serial.begin(115200);
#if (defined ESP32)
  DF1201SSerial.begin(115200, SERIAL_8N1, /*rx =*/17, /*tx =*/16);
#else
  ...
  // pinMode(touchLEDPin, INPUT);

  // digitalWrite(touchLEDPin, 0);
...
void loop()
{
  int currTouch = touchRead(T0);
  int btnVolUp = digitalRead(btnVolUpPin);
  int btnVolDown = digitalRead(btnVolDownPin);

  // Serial.print("Vol up button: ");
  // Serial.println(btnVolUp);

  if (currTouch < threshold && !prevTouch) {
    Serial.println("TOUCHED");
    // digitalWrite(touchLEDPin, 1);
